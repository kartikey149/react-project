...
const Navbar = () => {
  ...
  const handleMouseEnter = () => {
    if (divcolorref.current) {
      divcolorref.current.style.height = '100%'
    }
  }

  const handleMouseLeave = () => {
    if (divcolorref.current) {
      divcolorref.current.style.height = '0%'
    }
  }

  return (
    <div className='fixed top-0 left-0 w-full flex items-center justify-between z-50  s'>
     ...
     <div className='h-full w-30  flex items-center justify-center'>
      ...
      <div 
        className='relative h-10 w-20 bg-black cursor-pointer ' 
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        {/* Inner green fill div */}
        <div 
          ref={divcolorref} 
          style={{ height: '0%' }}
          className='absolute top-0 left-0 w-full bg-[#22c55e] transition-all duration-300 ease-in-out'
        ></div>
      </div>
    </div>
  )
